...
`kubectl exec` 可以进到 pod 容器内获取 shell，有时候需要 shell 到 node，而 原生的 `kubectl debug` 虽然支持但是不支持特权模式，拿不到 root 权限，也不能用宿主机上的大部分工具，发现 `Lens` 上有这个功能，把核心的部分拷贝出来，做成脚本，作为 `kubectl` 插件。
